...
# CSS REFINADO (ALINHAMENTO DA IMAGEM 2 E BOTÕES CONTIDOS)
CV_CSS = """
<style>
    body { background-color: #f4f4f4; display: flex; justify-content: center; padding: 40px 0; font-family: 'Arial', sans-serif; }
    .cv-paper { background-color: #ffffff; width: 850px; padding: 50px; box-shadow: 0 0 15px rgba(0,0,0,0.1); color: #000; line-height: 1.4; }
    ...
    /* Tabelas para garantir alinhamento Empresa/Data na Exportação (Imagem 2) */
    .timeline-table { width: 100%; border-collapse: collapse; margin-top: 15px; }
    .company-name { text-align: left; font-weight: bold; font-size: 1.1em; }
    .date-range { text-align: right; font-weight: bold; font-size: 1.1em; }
...
# Criando o container centralizado para os inputs e o botão
...
with center_col:
    ...
    # BOTÃO CORRIGIDO: Agora dentro da coluna central e sem use_container_width=True para não explodir
...
if btn_gerar:
    if uploaded_file and job_description.strip():
        st.session_state.content = ""
        with st.spinner("Formatando conforme padrão profissional..."):
            try:
                ...
                ESTRUTURA OBRIGATÓRIA (FORMATO IMAGEM 2):
                1. No [CV_START]:
                   - Nome e contatos centralizados no topo.
                   - Para Experiências: use <table> com largura 100%. 
                     - Linha 1: Empresa (esquerda) e Datas (direita) em Negrito.
                     - Linha 2: Cargo (esquerda) e Local (direita) em Itálico.
                   - Descrição: Bloco de texto corrido separando itens por "/", sem bullets.
                2. No [ANALYSIS_START]: análise detalhada com seções de melhoria e perguntas.
...
if st.session_state.content:
    ...
    if cv_match:
        ...
        full_document = f"<html><head><meta charset='UTF-8'>{CV_CSS}</head><body><div class='cv-paper'>{clean_html}</div></body></html>"
        
        components.html(full_document, height=1000, scrolling=True)

        # Botão de Exportação centralizado
        _, btn_col, _ = st.columns([1, 2, 1])
        with btn_col:
            st.download_button(
                label="📥 Baixar Currículo Formatado (.doc)",
                data=full_document,
                file_name="Curriculo_Profissional.doc",
